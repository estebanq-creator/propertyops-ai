...
describe('Laura Portal Compliance Tests', () => {
  const pagesToCheck = [
    ...
    'src/app/landlord/reports/page.tsx', // if exist
  ];
  ...
  it('Approve API route creates audit log entry', async () => {
    ...
    const mockRequest = new Request('http://example.com/logout');
    const params = { params: { id: 'test-task' } } as any;
    await POST(mockRequest, params);

    const calls = (global as any).__paperclipCalls;
    const auditCall = calls.find((c: any) => c.url.endsWith('/audit-logs'));
