...
12. Read `agents/ceo/PHASE0-PROTOCOLS.md` if it exists — contains Phase 0 KPI measurement protocol and Referral Engine triggers; required while any Phase 0 account is active
13. Read `USER.md`
14. Read `memory/YYYY-MM-DD.md` for today and yesterday if available
15. If in a direct main session with David, also read `MEMORY.md` and `memory/nightly/latest.qmd` if it exists
16. Read `paperclip-api-key.json` — this is your Paperclip API key. Use `token` as the Bearer token and `baseUrl` for all Paperclip API calls (issues, agents, runs). Example: `curl -s -H "Authorization: Bearer <token>" <baseUrl>/api/...`
